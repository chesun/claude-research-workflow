...
1. **Built-in blocklist (`NEVER_SURNAMES`).** Hard-coded set of words that are *never* surnames in academic prose: function words ("the", "in", "from"), seasons ("spring", "summer"), months, days of the week, document-structure words ("table", "figure", "section", "panel", "cohort"), pronouns, role-placeholder words used in citation-style examples ("Author", "Authors", "Coauthor", "Editor", "Name", "Surname"), book/series-title nouns ("Handbook", "Methodology", "Encyclopedia", "Annual", "Bulletin", "Journal", "Review", "Volume", "Issue"), mixed-case ADR/workflow status words ("Decided", "Proposed", "Superseded", "Pending", "Deferred", "Open", "Status", ...), and changes-table verbs ("Added", "Fixed", "Removed", "Merged", "Dropped", "Patched", ...). These drop regardless of allowlist state. Eliminates the common false-positive classes ("Spring 2015", "Table 2 (2024)", "Cohort 2018", "From 1999 onward...", "Author and Author (year)", "Handbook of Experimental Methodology 2025", "Decided 2026-06-03", "Fixed (2026)") without any project configuration.
...
The extractor ASCII-folds text before matching (NFD-decompose + strip combining marks, plus a precomposed-letter map for `ł ø ß æ œ ð þ`), so accented surnames (`Székely`, `García`, `Müller`, `Bénabou`, `Łukasz`) match as their ASCII forms. Reading-notes filenames should use the ASCII stems — `szekely_rizzo_2013.md`, not `székely_rizzo_2013.md`. The notes/PDF lookups also try a hyphen→underscore stem fallback, so a 2-part hyphenated dual-author citation (stem `szekely-rizzo_2013`) resolves to the conventional underscore filename, and accented `**Citation:**` metadata lines are folded before matching.
