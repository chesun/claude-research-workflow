...
- [2026-05-29_evidence-gating-build-plan.md](2026-05-29_evidence-gating-build-plan.md) — DRAFT (awaiting approval): build plan for the unified evidence-gating discipline (design of record `reviews/2026-05-28_whole-picture-critic-gates-dispatch.md` §7). 5 phases: Tier-1 language-agnostic normdiff hook (Stata/Python/R/LaTeX; register broadly, act narrowly via `refactor-mode`) → the one rule + verdict vocabulary + ledger schema → Tier-2 schema evidence + citation existence-check → operationalization gate (advisory) + Tier-3 panel → propagation. On branch `design/evidence-gating-discipline`.
