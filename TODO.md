...
- [ ] **Evidence-Gating Discipline — Phase 2+** (design `quality_reports/reviews/2026-05-28_whole-picture-critic-gates-dispatch.md` §7; build plan `quality_reports/plans/2026-05-29_evidence-gating-build-plan.md`; branch `design/evidence-gating-discipline`). **Phase 1 DONE** (`8176d0f`: always-on normdiff recorder, Stata/R/Python/LaTeX). Remaining: **Phase 2** (rule + `{PASS,UNVERIFIED,FAIL}` vocabulary + ledger schema + critic-consumes-ledger = the GATE), Phase 3 (Tier-2 schema evidence + citation existence-check), Phase 4 (operationalization gate advisory + Tier-3 panel), Phase 5 (class-aware propagation: rule/hooks/lib = Class A; CLAUDE.md + workflow.md = Class B manual). Branch not pushed.
...
- [ ] **Evidence-gating Phase-1 minor polish** (non-blocking review findings): (a) `normdiff_lib.py` block-comment "symmetric clipping" docstring claim is imprecise for asymmetric edits — soften, or make block-strip string-literal-aware; (b) R/Python path-token regex can tokenize a non-path first quoted arg (e.g. `pd.read_csv(sep=",", ...)`) — guard against a `keyword=` before the quote or whitelist path-like content.
