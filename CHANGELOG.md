...
- **Evidence-gating discipline** — operationalizes `adversarial-default`: a verdict is only as good as the evidence behind it, and the verification mechanism scales to how checkable the claim is. Prompted by a real incident where a critic passed a contaminated "no-logic-change" mechanical refactor. Built phase by phase, each phase adversarially reviewed by a multi-agent workflow and independently re-verified; those reviews caught real defects at every phase. Design of record: `quality_reports/reviews/2026-05-28_whole-picture-critic-gates-dispatch.md` §7; build plan: `quality_reports/plans/2026-05-29_evidence-gating-build-plan.md`.
  - **Tier-1 recorder** (`.claude/hooks/normdiff_lib.py` + `evidence-gate-recorder.py`, PostToolUse): records a normalized-content diff (`no-logic-change` ledger row) on each edit to a research artifact. Silent, non-blocking, fail-open. Language-agnostic core plus per-language normalizers (Stata/R/Python/LaTeX). Scope is per-repo configurable via an `**Analysis roots:**` header in CLAUDE.md (default-fallback, path-traversal-hardened). `/tools normdiff` CLI.
  - **The gate** (`.claude/agents/coder-critic.md`): consults the ledger at review time; a recorded residue forbids a clean-refactor PASS. Verdict vocabulary `{PASS, UNVERIFIED, FAIL}`. Gate on claims, not actions: evidence is gathered continuously, gating happens when a claim is made.
  - **Tier-2** (`.claude/hooks/citation_existence_lib.py`, `/tools cite-check`): critic verdicts require resolvable `{claim, artifact_citation, sufficiency_argument}`; the citation-existence check catches fabricated artifacts (RESOLVED/MISSING/ASSUMED). Security-hardened: repo-confined paths, whitelisted runner, no shell.
  - **Tier-3 and operationalization gate** (advisory): `.claude/references/evidence-gating-tier3-panel.md` (independent adversarial verification for load-bearing judgment verdicts) plus a Verification-Tier column in `templates/requirements-spec.md` and `.claude/rules/workflow.md` §1.
  - **Rule**: `.claude/rules/adversarial-default.md` gains the principle, verdict vocabulary, enforcement-strength rule, and the known limit (external-editor and `git commit --no-verify` bypass); the detail lives in `.claude/references/evidence-gating-detail.md`.
  - Distributed to all 7 consumer repos and both overlay branches, each scoped to its real analysis layout (e.g. `tx_peer_effects_local` to `do/`).
  - **Stata normalizer hardening** (`0d98330`) — fixed 4 mechanical-refactor false-UNVERIFIED escapes found on real files: `regsave` output paths, §4 scaffolding (`di`/`local dofile`/`assert strpos`/the multi-line required-globals guard), dead-local removal, and unquoted absolute paths. Stata-only; +8 regression tests (incl. injected-logic, consumer-change, division, and real-foreach negatives); fail-closed verified.
